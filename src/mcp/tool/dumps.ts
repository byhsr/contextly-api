...
export function registerDumpTools(server: any, db: any, userId: string) {
 server.registerTool("create_dump", {
  description: "Create a new dump under a context node",
...
}, async ({ contextId, content, actorType, actorId }: { contextId: string; content: string; actorType: string; actorId?: string }) => {
  ...
  const dump = { id: globalThis.crypto.randomUUID(), contextId, userId, actorType, actorId: actorId ?? userId, content, createdAt: new Date().toISOString() };
